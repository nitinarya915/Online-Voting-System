...
public class Dao {
    ...
    static
    {
        try
        {
            ...
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/evoting", "root", "Nitinary@915");
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public static int register(Model m) throws SQLException{
        int result =0;
        String sql="insert into login(voter_card_number,uname,username,gender,dob,email,password) values(?,?,?,?,?,?,?)";
        PreparedStatement ps=con.prepareStatement(sql);
        ps.setString(1, m.getVoterId());
